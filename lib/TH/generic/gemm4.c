...
sgemm_opt(const uint64_t m, const uint64_t n, const uint64_t p,
          const float pAlpha, const float * restrict A, const float * restrict B,
...
#pragma omp parallel num_threads(TNUM)
  {
    ...
    uint64_t block_id_i = tid >> 5;
    uint64_t block_id_j = tid & 0x1f;

    uint64_t thread_size_i = m / (TNUM >> 5);
    uint64_t thread_size_j = n / (TNUM >> 1) + 1;
      ...
      for (uint64_t tid_i = 0; tid_i < thread_size_i; tid_i += LI) {
        ...
        for (uint64_t kk = 0; kk < p; kk += LK) {
          ...
          for (uint64_t j = jj; j < jj + LJ; j += 16) {
            ...
            for (uint64_t i = ii; i < ii + LI; i += 16) {
